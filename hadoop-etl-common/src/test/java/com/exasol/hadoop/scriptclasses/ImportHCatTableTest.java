...
public class ImportHCatTableTest {
    ...
    public void testGenerateSqlForImportSpecSubselect() {
        ...
        String sqlExpected =
            ...
            + " "
            + meta.getScriptSchema()
            + ".IMPORT_HIVE_TABLE_FILES(hdfspath, input_format, serde, column_info, "
            + "       partition_info, serde_props, hdfs_server_port, "
            + "       hdfs_user_or_service_principal, auth_type, conn_name, output_columns, "
            + "       enable_rpc_encryption, debug_address)"
            + " EMITS (\"c1\" DECIMAL(16,0),\"c2\" VARCHAR(1000)) FROM ("
            + " SELECT "
            + meta.getScriptSchema()
            + ".HCAT_TABLE_FILES('hcat_db', 'hcat_table', 'hcat_address', "
            + "     'hdfs_service_principal', 'hcat_service_principal', nproc(), "
            + "     'p1=01', 'f1[0],f2', 'hdfs://custom', 'kerberos', 'MyKerberosConn', "
            + "     'false', 'host:1234')"
            + ") GROUP BY import_partition;";
        sqlExpected = normalizeSql(sqlExpected);
    ...
    public void testGenerateSqlForImportSpecInvalidSubselect() {
        ...
        thrown.expectMessage("In case of IMPORT in a subselect you need to specify the "
                + "output columns, e.g. 'INSERT INTO (a int, b varchar(100)) ...'.");
    ...
    public void testGenerateSqlForImportSpecMissingProperty() {
        ...
        thrown.expect(RuntimeException.class);
        thrown.expectMessage("The mandatory property HCAT_DB was not defined. "
                + "Please specify it and run the statement again.");
    ...
    /**
     * Convert newlines, tabs, and double whitespaces to whitespaces.
     *
     * <p>At the end only single whitespaces remain.
     */
