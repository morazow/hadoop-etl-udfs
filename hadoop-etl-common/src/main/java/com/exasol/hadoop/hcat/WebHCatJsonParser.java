...
public class WebHCatJsonParser {
  ...
  public static HCatTableMetadata parse(String json) throws Exception {
    ...
    String serDe =
        response.getJsonObject("sd").getJsonObject("serdeInfo").getString("serializationLib");
    List<HCatSerDeParameter> serDeParameters =
        parseSerDeParameters(
            response.getJsonObject("sd").getJsonObject("serdeInfo").getJsonObject("parameters"));
  ...
  public static List<HCatSerDeParameter> parseSerDeParameters(String json) throws Exception {
    ...
    for (Entry<String, JsonValue> param : obj.entrySet()) {
      ...
      serDeParameters.add(
          new HCatSerDeParameter(param.getKey(), ((JsonString) param.getValue()).getString()));
    }
    return serDeParameters;
  }
}
