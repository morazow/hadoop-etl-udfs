...
public class ExportHCatTable {
    ...
    public static String generateSqlForExportSpec(
            ExaMetadata meta, ExaExportSpecification exportSpec) {
        ...
        final String hdfsAddress = getParameter(params, "HDFS_URL", "");
        ...
        final String compressionType = getParameter(params, "COMPRESSION_TYPE", "uncompressed");
        ...
        if (staticPartition != null
                ...
                && dynamicPartitionExaCols != null
                && !dynamicPartitionExaCols.isEmpty()) {
        ...
        if (dynamicPartitionExaCols != null && !dynamicPartitionExaCols.isEmpty()) {
            ...
            dynamicPartsExaColNums = getExaColumnNumbersOfSpecifiedDynamicPartitions(
                    dynamicPartitionExaCols, exaColNames);
        } else {
            ...
            if (!kerberosConnection.isEmpty()) {
                ...
                try {
                    ...
                    throw new RuntimeException(
                            "Exception while accessing Kerberos connection "
                            + kerberosConnection
                            + ": "
                            + e.toString(),
                            e);
            ...
            try {
                if (isUnitTestMode) {
                    ...
                    tableMeta = new HCatTableMetadata(
                            "", new ArrayList<>(),
                            new ArrayList<>(), "", "", "", "", new ArrayList<>());
                } else {
                    tableMeta = HCatMetadataService.getMetadataForTable(
                            hcatDB,
                            hcatTable,
                            hcatAddress,
                ...
                throw new RuntimeException("Exception while fetching metadata for "
                        + hcatTable + ": " + e.toString(), e);
            }
            dynamicPartsExaColNums =
                getExaColumnNumbersOfNonSpecifiedDynamicPartitions(tableMeta, exaColNames.size());
        }
        ...
        // Note: The FROM clause of the following SQL statement will be replaced in Exasol with
        // the FROM clause of the original SQL statement (which called this UDF). Thus, only a
        // dummy placeholder value needs to be specified here.
        ...
        sql.append(") ");
        sql.append("FROM ");
        // Dummy placeholder, will be replaced by the query or table to be exported.
        sql.append("DUAL");
    ...
    private static List<Integer> getExaColumnNumbersOfSpecifiedDynamicPartitions(
            String dynamicPartitionExaCols, List<String> exaColNames) {
        ...
        for (String dynamicCol : dynamicCols) {
            ...
            if (exaColIndex == -1) {
                throw new RuntimeException(
                        "Dynamic partition " + dynamicCol + " was not found in column list");
            }
    ...
    private static List<Integer> getExaColumnNumbersOfNonSpecifiedDynamicPartitions(
            HCatTableMetadata tableMeta, int numExaCols) {
        List<Integer> dynamicPartsExaColNums = new ArrayList<>();
        ...
        if (numHivePartitions > 0) {
            if (numHivePartitions > numExaCols) {
                throw new RuntimeException(
                        ...
                        + numHivePartitions
                        + " although the source table only has "
                        + numExaCols
                        + " partitions.");
            }
    ...
    private static ExaConnectionInformation getJdbcConnection(
            ExaMetadata meta, String jdbcConnection) {
        if (jdbcConnection.isEmpty()) {
            throw new RuntimeException(
                    "The JDBC_CONNECTION parameter is required, but was not specified.");
        }
        ...
        try {
            ...
            throw new RuntimeException(
                    "ExaConnectionAccessException while getting connection "
                    + jdbcConnection
                    + ": "
                    + e.toString(),
                    e);
    ...
    private static List<String> getJdbcStatements(
            ExaExportSpecification exportSpec, String hcatDB, String hcatTable) {
