...
/**
 * Retrieves files from (web)HDFS and decodes them using the appropriate
 * Hadoop InputFormat and Hive SerDe.
 *
 * <p>TODO org.apache.hadoop.hive.serde2.SerDe is deprecated and should
 * be replaced with org.apache.hadoop.hive.serde2.AbstractSerDe Problem:
 * Some SerDes like OrcInputFormat support only deprecated SerDe class
 * (not AbstractSerDe)
 */
public class HdfsSerDeImportService {
    ...
    public static void importFiles(
        ...
        try {
            ...
            final List<OutputColumnSpec> outputColumns = outputColumnsSpec.isEmpty()
                ? OutputColumnSpecUtil.generateDefaultOutputSpecification(
            ...
            ugi.doAs(new PrivilegedExceptionAction<Void>() {
                public Void run() throws Exception {
                    final Configuration conf = HdfsService.getHdfsConfiguration(
     ...
     * Imports a file.
     *
     * @return In case of an HA failover, this method might connect to a
     *         different namenode, which is then returned
     */
    public static FileSystem importFile(
        ...
        System.out.println("- hdfsUrl: "
                + org.apache.commons.lang.StringUtils.join(hdfsUrls, ","));
        ...
        // If there is a HA namenode failover while UDFs are running, it
        // will probably crash here when retrieving the files from
        // namenode.
        // In case of a crash, we call {@HdfsService#getFileSystem}
        // which will get the available namenode (or it fails because
        // all are down)
        try {
        ...
        InputSplit split = new FileSplit(
                fileStatus.getPath(), 0, fileStatus.getLen(), (String[]) null);
        ...
        // Convert from URL encoding. TODO Probably we should only do this for webHDFS
        String decodedFilePath = URLDecoder.decode(file, "UTF-8");
        setPartitionValues(
                decodedFilePath, columns.size(), partitionColumns, outputColumns, objs);
        ...
        while (rr.next(key, value)) {
            // In complex type hierarchies with null values, some columns might not be set, so we
            // have to set manually.
            // TODO Find a better which is optimized for the case where we have to nullify only
            // the fields which were not already overwritten.
            ...
            for (int i = 0; i < columns.size(); i++) {
                ...
                try {
                    ...
                    // This can be reproduced by creating a Parquet table, inserting data, and
                    // afterwards adding columns.
                    // Then, when reading the files created before the columns were added, you get
                    // the Exception.
                    objs[i] = null;
                } catch (Exception e) {
                    throw new RuntimeException(
                            "Unhandled exception while getting column " + i, e);
                }
            }
            ctx.emit(objs);
        }
    ...
    private static void visitTree(
        ...
        for (OutputColumnSpec spec : outputColumns) {
            if (spec.getPath().equals(curPath)) {
                switch (objInspector.getCategory()) {
                    case PRIMITIVE:
                        // We reached a leaf. Path should match completely (We could show an error
                        // if there are deeper paths for this subtree)
                        outRow[spec.getResultPosition()] = getJavaObjectFromPrimitiveData(
                                data, objInspector);
                        break;
                    case LIST:
                        outRow[spec.getResultPosition()] = getJsonArrayFromFieldData(
                                data, objInspector, jsonFactory).build().toString();
                        break;
                    case MAP:
                    case STRUCT:
                    case UNION:
                        outRow[spec.getResultPosition()] = getJsonObjectFromFieldData(
                                data, objInspector, jsonFactory).build().toString();
                        break;
                    default:
                        break;
        ...
        while (objInspector.getCategory() == Category.UNION) {
            // Type may vary for each row, we have to be careful. If we vectorize and we find a
            // union we have to set vector size to 1.
        ...
        switch (objInspector.getCategory()) {
            case LIST: {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    visitTree(elementData,
                            elementInsp, curPath, outputColumns, outRow, jsonFactory);
            ...
            case MAP: {
                ...
                for (Map.Entry<?, ?> mapEntryData : mapData.entrySet()) {
                    if (keyInspector.getCategory() != Category.PRIMITIVE) {
                        throw new RuntimeException("Hive map key is not a primitive type: "
                                + keyInspector.getCategory());
                    }
                    String key = getJavaObjectFromFieldData(
                            mapEntryData.getKey(), keyInspector).toString();
            ...
            default: {
                throw new RuntimeException("Unexpected category "
                        + objInspector.getCategory() + ", should never happen");
            }
        }
    }
    ...
    private static Object getJavaObjectFromPrimitiveData(Object data, ObjectInspector objInsp) {
        ...
        if (data instanceof BytesWritable
                && objInsp instanceof WritableHiveDecimalObjectInspector) {
            ...
            case MAP: {
                ...
                for (Map.Entry<?, ?> entry : map.entrySet()) {
                    if (keyInsp.getCategory() != Category.PRIMITIVE) {
                        throw new RuntimeException("Hive map key is not a primitve type: "
                                + keyInsp.getCategory());
                    }
                    String key = getJavaObjectFromFieldData(
                            entry.getKey(), keyInsp).toString();
                    addJsonObjectPair(job, key, entry.getValue(),
                            oi.getMapValueObjectInspector(), jsonFactory);
                }
                break;
            }
            case STRUCT: {
                ...
                for (int i = 0; i < size; i++) {
                    StructField field = structFields.get(i);
                    String name = field.getFieldName();
                    addJsonObjectPair(job, name, structValues.get(i),
                            field.getFieldObjectInspector(), jsonFactory);
                }
                break;
            }
            case UNION: {
    ...
    private static void setPartitionValues(
        ...
        for (OutputColumnSpec outputColumn : outputColumns) {
            if (outputColumn.getColumnPosition() >= numRegularColumns) {
                ...
                if (partIdx < 0) {
                    throw new RuntimeException("Hive partition '"
                            + partName + "' could not be found in path: " + file);
                }
    ...
    private static Object getJavaObjectFromHiveString(
        ...
        switch (javaType) {
            ...
            case "java.lang.Boolean":
                ...
                break;
            default:
                throw new RuntimeException("Hive type '"
                        + type + "' is unsupported for partitions");
        }
        return obj;
    }
