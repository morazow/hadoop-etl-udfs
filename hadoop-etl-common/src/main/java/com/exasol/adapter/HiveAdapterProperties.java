...
public class HiveAdapterProperties {
    ...
    private static String getProperty(
            Map<String, String> properties, String name, String defaultValue) {
    ...
    /**
     * Returns the credentials for the remote system. These are either directly specified in the
     * properties or obtained from a connection (requires privilege to access the connection .
     */
    public static ExaConnectionInformation getConnectionInformation(
            Map<String, String> properties, ExaMetadata exaMeta) {
        String connName = getProperty(properties, PROP_CONNECTION_NAME, "");
        if (connName != null && !connName.isEmpty()) {
            try {
                ...
                throw new RuntimeException(
                        "Could not access the connection information of connection "
                        + connName
                        + ". Error: "
                        + e.toString());
            }
    ...
    /**
     * Returns the properties as they would be after successfully
     * applying the changes to the existing (old) set of properties.
     */
    public static Map<String, String> getNewProperties(
        ...
        for (Map.Entry<String, String> changedProperty : changedProperties.entrySet()) {
            if (changedProperty.getValue() == null) {
                // Null values represent properties which are deleted by
                // the user (might also have never existed actually)
