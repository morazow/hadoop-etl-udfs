...
/**
 * Utility Methods to work with a multi partition filter, i.e. user specified multiple filters
 * (logically connected by OR)
 */
  ...
  private static boolean matchesSingleFilter(
    ...
    for (String partition : partitionValues.keySet()) {
      if (filter.containsPartition(partition)
          && !filter.getPartitionValue(partition).equals(partitionValues.get(partition))) {
        return false;
      }
    }
    return true;
