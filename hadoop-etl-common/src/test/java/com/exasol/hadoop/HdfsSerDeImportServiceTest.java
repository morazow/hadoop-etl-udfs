...
public class HdfsSerDeImportServiceTest {
  ...
  public void testImportSample07Rc() throws Exception {
    // test resources from src/test/resources are copied to target/test-classes, and this folder is
    // the classpath of the junit test. However, classpath doesn't help here, so we need to hardcode
    // the path.
  ...
  public void testImportComplexRCFileWithJsonPath() throws Exception {
    ...
    verify(ctx, times(1))
        .emit(
            ...
            eq("C"),
            eq(null));
    // Structs with non-existing fields are mistakenly returned as arrays with a single null from
    // SerDe. Seems to be a SerDe bug. Hive correctly shows NULL instead of [null] as we do
    verify(ctx, times(1))
        .emit(
            eq("F1-a"),
            eq(null),
