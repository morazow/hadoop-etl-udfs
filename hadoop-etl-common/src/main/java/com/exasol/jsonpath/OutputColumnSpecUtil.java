...
public class OutputColumnSpecUtil {
    ...
    /**
     * Parses output specification.
     *
     * @param outputSpecification A comma separated list of output
     *        columns and optional JsonPath suffixes. The whole path is case
     *        sensitive.
     */
    public static List<OutputColumnSpec> parseOutputSpecification(
        ...
        try {
            ...
            for (int i = 0; i < elements.length; ++i) {
                ...
                if (path.get(0).getType() != JsonPathElement.Type.FIELD) {
                    throw new IllegalArgumentException(
                            "Could not parse output column specification. Invalid element "
                            + i
                            + ": "
                            + elements[i]);
                }
                ...
                int columnPosition =
                    findColumnPositionByName(columnName, columns, partitionColumns);
                if (path.size() == 1) {
            ...
            throw new IllegalArgumentException(
                    "Could not parse output column specification due to error '"
                    + e.toString()
    ...
    public static List<OutputColumnSpec> parseOutputSpecification(
            String outputSpecification, List<HCatTableColumn> columns) {
        return parseOutputSpecification(
                outputSpecification, columns, new ArrayList<HCatTableColumn>());
    }

    private static int findColumnPositionByName(
            String columnName, List<HCatTableColumn> columns, List<HCatTableColumn> partitions) {
        ...
        throw new RuntimeException("Could not find column "
                + columnName + ". Note that the columns are case-sensitive");
    }

    /**
     * Checks whether the left path is prefic of the right path.
     *
     * <p>Must be a real prefix.
     *
     * <p>Example: Return true if left path is 'field1[0]', and the
     * right path is 'field1[0].field2.
     */
    public static boolean leftIsPrefixOfRightPath(
            List<JsonPathElement> leftPath, List<JsonPathElement> rightPath) {
    ...
    /** Returns specification for output columns. */
    public static Map<Integer, List<OutputColumnSpec>> getOutputSpecsForColumns(
    ...
    /** Returns default specification for output columns. */
    public static List<OutputColumnSpec> generateDefaultOutputSpecification(
            List<HCatTableColumn> columns, List<HCatTableColumn> partitionColumns) {
        List<OutputColumnSpec> columnsSpec = new ArrayList<>();
        for (int i = 0; i < columns.size(); i++) {
            columnsSpec.add(new OutputColumnSpec(i, columns.get(i).getName(), i));
        }
        for (int i = 0; i < partitionColumns.size(); i++) {
            columnsSpec.add(new OutputColumnSpec(
                        i + columns.size(), partitionColumns.get(i).getName(),
                        i + columns.size()));
        }
        return columnsSpec;
    }
