...
public class HiveAdapterProperties {
  ...
  /**
   * Returns the properties as they would be after successfully applying the changes to the existing
   * (old) set of properties.
   */
  public static Map<String, String> getNewProperties(
    ...
    for (Map.Entry<String, String> changedProperty : changedProperties.entrySet()) {
      if (changedProperty.getValue() == null) {
        // Null values represent properties which are deleted by the user (might also have never
        // existed actually)
