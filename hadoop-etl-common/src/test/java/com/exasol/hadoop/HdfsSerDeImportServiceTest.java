...
public class HdfsSerDeImportServiceTest {
    ...
    public void testImportSample07Rc() throws Exception {
        // test resources from src/test/resources are copied to target/test-classes, and this
        // folder is the classpath of the junit test. However, classpath doesn't help here, so we
    ...
    public void testImportSample07RcSmall() throws Exception {
        ...
        verify(ctx, times(expectedNumRows)).emit(anyVararg());
        verify(ctx, times(1)).emit(eq("00-0000"),
                eq("All Occupations"), eq(134354250), eq(40690));
        verify(ctx, times(1)).emit(eq("11-0000"),
                eq("Management occupations"), eq(6003930), eq(96150));
        verify(ctx, times(1)).emit(eq("11-1011"), eq("Chief executives"), eq(299160), eq(151370));
        verify(ctx, times(1))
            .emit(eq("11-1021"), eq("General and operations managers"), eq(1655410), eq(103780));
    ...
    public void testImportSample07RcSmallMixCols() throws Exception {
        ...
        List<OutputColumnSpec> outputColumns =
            OutputColumnSpecUtil.parseOutputSpecification(
                    "salary, total_emp, description, code, salary", columns);
        runImportRCFile(ctx, columns, new ArrayList<HCatTableColumn>(), outputColumns, file);

        int expectedNumRows = 5;
        verify(ctx, times(expectedNumRows)).emit(anyVararg());
        verify(ctx, times(1))
            .emit(eq(40690), eq(134354250), eq("All Occupations"), eq("00-0000"), eq(40690));
        verify(ctx, times(1))
            .emit(eq(96150), eq(6003930), eq("Management occupations"), eq("11-0000"), eq(96150));
        verify(ctx, times(1))
            .emit(eq(151370), eq(299160), eq("Chief executives"), eq("11-1011"), eq(151370));
        verify(ctx, times(1))
            .emit(
                    eq(103780),
                    eq(1655410),
                    eq("General and operations managers"),
                    eq("11-1021"),
                    eq(103780));
        verify(ctx, times(1)).emit(eq(33880),
                eq(61110), eq("Legislators"), eq("11-1031"), eq(33880));
    }

    private void runImportRCFile(
            ExaIterator ctx,
    ...
    public void testImportComplexRCFile() throws Exception {
        ...
        verify(ctx, times(1))
            .emit(
                    ...
                    eq("[[\"A\",\"B\"],[\"C\",\"D\"]]"),
                    eq("{\"g1\":{\"h1\":{\"K1\":\"V1\",\"K2\":\"V2\"}}}"));
        // TODO Check what to do with \N values. They encoding a NULL value. See
    ...
    public void testImportComplexRCFileWithJsonPath() throws Exception {
        ...
        List<OutputColumnSpec> outputColumns =
            OutputColumnSpecUtil.parseOutputSpecification(
                    "f1[0], f1[1], f2.K1, f3.g2, f4, f5, f5[1][0], f6.g1.h1.k1", columns);
        runImportRCFile(ctx, columns, new ArrayList<HCatTableColumn>(), outputColumns, file);
        ...
        verify(ctx, times(1))
            .emit(
                    ...
                    eq("C"),
                    eq(null));
        // Structs with non-existing fields are mistakenly returned as arrays with a single null
        // from SerDe. Seems to be a SerDe bug. Hive correctly shows NULL instead of [null] as we
        // do
        verify(ctx, times(1))
            .emit(
                    eq("F1-a"),
