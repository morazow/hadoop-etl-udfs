...
public class HdfsService {
    ...
    public static List<String> getFilesFromTable(
        ...
        String loginUser =
            useKerberos ? kerberosCredentials.getPrincipal() : hdfsUserOrServicePrincipal;
        ...
        List<String> tableInfo = ugi.doAs(new PrivilegedExceptionAction<List<String>>() {
            ...
            public List<String> run() throws Exception {
                ...
                List<String> partitionPaths = getPartitionPaths(
                        fs,
                        tableRootPath,
                        partitionColumns,
                        MultiPartitionFilter.parseMultiFilter(partitionFilterSpec));
    ...
    public static FileSystem getFileSystem(List<String> hdfsURLs, Configuration conf)
            throws IOException {
        ...
        for (String hdfsURL : hdfsURLs) {
            try {
                ...
                // Dirty hack: The above creation of the filesystem does not actually connect to
                // the hdfs namenode, so it does not fail if the hostname is good but hdfs is not
                // listening on the specified port.
                // So we need to do something (cheap) with the filesystem to check if it is really
                // available.
                try {
        ...
        throw new RuntimeException(
                "None of the provided HDFS URLs is reachable: " + exceptions, lastException);
    }

    /**
     * Returns the partition paths.
     *
     * @param rootPath Root directory of the table, e.g.
     *                 "/user/hive/warehouse/albums_rc_multi_part"
     */
    static List<String> getPartitionPaths(
            FileSystemWrapper fs,
            String rootPath,
            List<HCatTableColumn> partitionColumns,
            List<PartitionFilter> filters) throws Exception {
        System.out.println(
                "call getPartitionPaths() for rootPath="
                + rootPath
                + " partitionColumns="
                + partitionColumns.toString()
                + " filters: "
                + filters.toString());
        // Will hold all directories which are part of the filter (only leave-directories at the
        // end)
        List<String> partitionPaths = new ArrayList<>(1);
        ...
        for (int partition = 0; partition < partitionColumns.size(); partition++) {
            ...
            PartitionPathFilter pathFilter =
                new PartitionPathFilter(partitionColumns, filters, partition + 1);
    ...
    private static List<String> getFilePaths(FileSystemWrapper fs, List<String> partitionPaths)
            throws IOException {
     ...
     *
     * @param useKerberos an indicator whether to use the kerberos or not
     * @param kerberosHdfsServicePrincipal if kerberos is true, then
     *        service principal for Hdfs, otherwise null or empty string
     */
    public static Configuration getHdfsConfiguration(
            boolean useKerberos, String kerberosHdfsServicePrincipal,
