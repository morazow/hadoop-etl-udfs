...
public class OutputColumnSpecUtil {
  ...
  private static int findColumnPositionByName(
    ...
    throw new RuntimeException(
        "Could not find column " + columnName + ". Note that the columns are case-sensitive");
  }
   ...
   *
   * <p>Example: Return true if left path is 'field1[0]', and the right path is 'field1[0].field2.
   */
  ...
  public static List<OutputColumnSpec> generateDefaultOutputSpecification(
    ...
    for (int i = 0; i < partitionColumns.size(); i++) {
      columnsSpec.add(
          new OutputColumnSpec(
              i + columns.size(), partitionColumns.get(i).getName(), i + columns.size()));
    }
    return columnsSpec;
  }
}
