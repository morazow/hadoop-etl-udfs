...
public class HdfsSerDeExportServiceTest {
    ...
    // UDF argument number that has the first data column (see ExportIntoHiveTable class)
    static final int FIRST_DATA_COLUMN = 11;

    static final String PARQUET_INPUT_FORMAT_CLASS_NAME =
        "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat";
    static final String PARQUET_SERDE_CLASS_NAME =
    ...
    public void testExportParquetNumeric() throws Exception {
        ...
        schemaTypes.add(new ExaParquetTypeInfo(
                    "decimalnull", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 4));
        ...
        File tempFile = new File(testFolder.getRoot(),
                UUID.randomUUID().toString().replaceAll("-", "") + ".parq");

        List<Integer> dynamicCols = new ArrayList<>();
        HdfsSerDeExportService.exportToParquetTable(
        ...
        importFile(
                ...
                PARQUET_INPUT_FORMAT_CLASS_NAME,
                PARQUET_SERDE_CLASS_NAME);
    ...
    public void testExportParquetTimestamp() throws Exception {
        ...
        ZonedDateTime zdtDefault1 =
            zdtUtc1.withZoneSameInstant(ZoneId.of(TimeZone.getDefault().getID().toString()));
        ZonedDateTime zdtDefault2 =
            zdtUtc2.withZoneSameInstant(ZoneId.of(TimeZone.getDefault().getID().toString()));
        int expectedNumRows = 1;
        verify(ctx, times(expectedNumRows)).emit(anyVararg());
        verify(ctx, times(1))
            .emit(
    ...
    public void testExportParquetBoolean() throws Exception {
        ...
        int expectedNumRows = 1;
        verify(ctx, times(expectedNumRows)).emit(anyVararg());
        verify(ctx, times(1)).emit(eq(Boolean.TRUE), eq(Boolean.FALSE), eq(null));
    }
    ...
    public void testExportDecimalPadding() throws Exception {
        ...
        schemaTypes.add(
                new ExaParquetTypeInfo("dec9neg", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 12));
        schemaTypes.add(new ExaParquetTypeInfo("dec10", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 14));
        schemaTypes.add(
                new ExaParquetTypeInfo("dec10neg", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 14));
        schemaTypes.add(new ExaParquetTypeInfo("dec11", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 16));
        schemaTypes.add(
                new ExaParquetTypeInfo("dec11neg", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 16));
        schemaTypes.add(new ExaParquetTypeInfo("dec12", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 4));
        schemaTypes.add(
                new ExaParquetTypeInfo("dec12neg", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 4));
        schemaTypes.add(new ExaParquetTypeInfo("dec13", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 5));
        schemaTypes.add(
                new ExaParquetTypeInfo("dec13neg", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 5));
        schemaTypes.add(new ExaParquetTypeInfo("dec14", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 10));
        schemaTypes.add(
                new ExaParquetTypeInfo("dec14neg", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 10));
        schemaTypes.add(new ExaParquetTypeInfo("dec15", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 16));
        schemaTypes.add(
                new ExaParquetTypeInfo("dec15neg", "OPTIONAL", "FIXED_LEN_BYTE_ARRAY", 16));

        List<Object> row = new ArrayList<>();
        row.add(new BigDecimal("1"));
        row.add(new BigDecimal("-1"));
        row.add(new BigDecimal("255"));
        row.add(new BigDecimal("-255"));
        row.add(new BigDecimal("256"));
        row.add(new BigDecimal("-256"));
        row.add(new BigDecimal("1023"));
        row.add(new BigDecimal("-1023"));
        row.add(new BigDecimal("64000"));
        row.add(new BigDecimal("-64000"));
        row.add(new BigDecimal("72000"));
        row.add(new BigDecimal("-72000"));
        row.add(new BigDecimal("16777211"));
        row.add(new BigDecimal("-16777211"));
        row.add(new BigDecimal("4294968234"));
        row.add(new BigDecimal("-4294968234"));
        row.add(new BigDecimal("87509812398888234"));
        row.add(new BigDecimal("-87509812398888234"));
        row.add(new BigDecimal("698234872384787230490982344234"));
        row.add(new BigDecimal("-698234872384787230490982344234"));
        row.add(new BigDecimal("871234870814250986098124309821039456"));
        row.add(new BigDecimal("-871234870814250986098124309821039456"));
        row.add(new BigDecimal("1.123"));
        row.add(new BigDecimal("-1.123"));
        row.add(new BigDecimal("6400.2"));
        row.add(new BigDecimal("-6400.2"));
        row.add(new BigDecimal("8750981239.8888234"));
        row.add(new BigDecimal("-8750981239.8888234"));
        row.add(new BigDecimal("0.871234870814250986098124309821039456"));
        row.add(new BigDecimal("-0.871234870814250986098124309821039456"));
        List<List<Object>> dataSet = new ArrayList<>();
        addRow(dataSet, row);
        ExaIterator iter = new ExaIteratorDummy(dataSet);
        ...
        verify(ctx, times(1)).emit(
                ...
                eq(new BigDecimal("698234872384787230490982344234")),
                eq(new BigDecimal("-698234872384787230490982344234")),
                eq(new BigDecimal("871234870814250986098124309821039456")),
                eq(new BigDecimal("-871234870814250986098124309821039456")),
                eq(new BigDecimal("1.123")), eq(new BigDecimal("-1.123")),
                eq(new BigDecimal("6400.2")), eq(new BigDecimal("-6400.2")),
                eq(new BigDecimal("8750981239.8888234")),
                eq(new BigDecimal("-8750981239.8888234")),
                eq(new BigDecimal("0.871234870814250986098124309821039456")),
                eq(new BigDecimal("-0.871234870814250986098124309821039456")));
    }
    ...
    private void importFile(
            ...
            String serdeName) throws Exception {
        ...
        InputFormat<?, ?> inputFormat =
            (InputFormat<?, ?>) UdfUtils.getInstanceByName(inputFormatName);
        ...
        List<OutputColumnSpec> outputColumns =
            OutputColumnSpecUtil.generateDefaultOutputSpecification(
                    columns, new ArrayList<HCatTableColumn>());
        HdfsSerDeImportService.importFile(
                fs,
                file,
                partitionColumns,
                inputFormat,
