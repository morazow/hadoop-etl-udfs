  ...
  public static String generateSqlForExportSpec(
    ...
    if (dynamicPartitionExaCols != null && !dynamicPartitionExaCols.isEmpty()) {
      ...
      dynamicPartsExaColNums =
          getExaColumnNumbersOfSpecifiedDynamicPartitions(dynamicPartitionExaCols, exaColNames);
    } else {
      ...
      try {
        ...
        throw new RuntimeException(
            "Exception while fetching metadata for " + hcatTable + ": " + e.toString(), e);
      }
      dynamicPartsExaColNums =
    ...
    // Note: The FROM clause of the following SQL statement will be replaced in Exasol with the FROM
    // clause
    //       of the original SQL statement (which called this UDF). Thus, only a dummy placeholder
    // value
