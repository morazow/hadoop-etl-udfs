...
public class OutputColumnSpec {
  // the position in the resultset for this column (starting with 0)
  int resultPosition;
  String columnName;
  // the position of the column in the original table (starting with 0). Might point to a
  // partition column.
  int columnPosition;
  ...
  /** Output column specification with json path. */
  public OutputColumnSpec(
      int resultPosition, String columnName, int columnPosition, List<JsonPathElement> path) {
    this.resultPosition = resultPosition;
    this.columnName = columnName;
    this.columnPosition = columnPosition;
    this.path = path;
