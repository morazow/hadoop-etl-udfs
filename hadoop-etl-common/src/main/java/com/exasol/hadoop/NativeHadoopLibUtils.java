...
public class NativeHadoopLibUtils {
  ...
  public static void initHadoopNativeLib()
    ...
    // This path must exist at program startup and must be in LD_LIBRARY_PATH. Otherwise the dynamic
    // linker seems to ignore it
  ...
  private static void writeNativeHadoopLibs(String targetLibDir) {
    try {
      ...
      System.out.println(
          "Error writing the native Hadoop libraries from resources to tmp: " + e.toString());
      e.printStackTrace();
    }
  }
  ...
  /**
   * This enables the java.library.path to be modified at runtime, which is required to load java
   * native libraries From a Sun engineer at http://forums.sun.com/thread.jspa?threadID=707176 See
  ...
  /**
   * This is another variant of addDirToJavaLibraryPath(). Currently unused, because the other
   * should work.
   */
