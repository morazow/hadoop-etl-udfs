...
public class HiveTableInformation {
    ...
    /** Return a hive table. */
    public static Table getHiveTable(
            HiveMetaStoreClient hiveMetastoreClient, String tableName, String databaseName) {
        Table table;
        try {
            ...
            throw new RuntimeException(
                    ...
                    + tableName + " in database " + databaseName + ": " + e.toString(), e);
        } catch (NoSuchObjectException e) {
            throw new RuntimeException(
                    "Table " + tableName + " in database " + databaseName
                    + ". Error: " + e.toString(), e);
    ...
    /** Returns a data type given a string hive type. */
    public static DataType typeMapping(String hiveType) throws SQLException {
        DataType colType;
        if (hiveType.toLowerCase().startsWith("array")
                || hiveType.toLowerCase().startsWith("map")
                || hiveType.toLowerCase().startsWith("struct")
                || hiveType.toLowerCase().equals("string")
                || hiveType.equals("binary")) {
            colType = DataType.createVarChar(
                    DataType.maxExasolVarcharSize, DataType.ExaCharset.UTF8);
            ...
            int size = new Integer(
                    hiveType.substring(hiveType.indexOf("(") + 1, hiveType.indexOf(")")));
            ...
            String sizeElements = hiveType.substring(
                    hiveType.indexOf("(") + 1, hiveType.indexOf(")"));
            int decimalPrecision = new Integer(
                    sizeElements.substring(0, sizeElements.indexOf(",")));
            int decimalScale = new Integer(
                    sizeElements.substring(sizeElements.indexOf(",") + 1));
    ...
    /** Return table metadata from HCat table metadata. */
    public static TableMetadata getTableMetadataFromHCatTableMetadata(
            String tableName, HCatTableMetadata hcatTableMetadata) throws MetadataException {
    ...
    public static void addColumsToList(
            List<ColumnMetadata> columns,
            List<HCatTableColumn> inputColums,
            boolean isPartition) {
        for (HCatTableColumn column : inputColums) {
            try {
                DataType dataType = typeMapping(column.getDataType());
                // TODO nice to have: derive nullable/isIdentity/defaultValue/comment from
                // HCatalog (not important)
                ColumnAdapterNotes columnAdapterNotes =
                    new ColumnAdapterNotes(column.getDataType(), isPartition);
                columns.add(new ColumnMetadata(
                            column.getName().toUpperCase(),
                            ColumnAdapterNotes.serialize(columnAdapterNotes),
                            dataType,
                            false,
                            false,
                            "",
    ...
    public static HCatTableMetadata getHCatTableMetadata(Table table) {
        ...
        return new HCatTableMetadata(sd.getLocation(),
                columns,
                partitionColumns,
                tableType,
                sd.getInputFormat(),
