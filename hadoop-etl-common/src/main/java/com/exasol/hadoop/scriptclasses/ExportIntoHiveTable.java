...
public class ExportIntoHiveTable {
    ...
    public static void run(ExaMetadata meta, ExaIterator iter) throws Exception {
        ...
        final String hdfsUserOrServicePrincipal =
            iter.getString(PARAM_IDX_HDFS_USER_OR_SERVICE_PRINCIPAL);
        String hcatUserOrServicePrincipal =
            iter.getString(PARAM_IDX_HCAT_USER_OR_SERVICE_PRINCIPAL);
        final String hdfsServerUrls =
            UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_HDFS_ADDRESS, "");
        final String staticPartition =
            UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_STATIC_PARTITION, "");
        final String dynamicPartitionExaCols = UdfUtils.getOptionalStringParameter(
                meta, iter, PARAM_IDX_DYNAMIC_PARTITION_EXA_COLS, "");
        String authType =
            UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_AUTH_TYPE, "");
        String connName =
            UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_AUTH_CONNECTION, "");
        String compressionType =
            UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_COMPRESSION_TYPE, "");
        final boolean enableRpcEncryption = UdfUtils.getOptionalStringParameter(
                meta, iter, PARAM_IDX_ENABLE_RPC_ENCRYPTION, "false")
            .equalsIgnoreCase("true");
        String debugAddress =
            UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_DEBUG_ADDRESS, "");
        ...
        if (!debugAddress.isEmpty()) {
            try {
                ...
                throw new RuntimeException("You have to specify a valid hostname and port "
                    + "for the udf debug service, e.g. 'hostname:3000'");
            }
        }
        ...
        if (!staticPartition.isEmpty() && !dynamicPartitionExaCols.isEmpty()) {
            throw new RuntimeException(
                    "A static and dynamic partition cannot both be specified.");
        } else if (!staticPartition.isEmpty()) {
            // Static partition
            if (hivePartitions.isEmpty()) {
                throw new RuntimeException("A static partition was specified although "
                        + "the target table does not have any partitions.");
            }
            ...
            throw new RuntimeException("A dynamic partition was specified although "
                    + "the target table does not have any partitions.");
        }

        // If the user defined an webHDFS URL (e.g. "webhdfs://domain.namenode:50070" or
        // "hdfs://namenode:8020") we use this and ignore the hdfs URL returned from HCat (e.g.
        // "hdfs://namenode:8020")
        // Two use cases: 1) Use webHDFS instead of HDFS and 2) If Hadoop returns a namenode
        // hostname unreachable from EXASOL (e.g. not fully-qualified) we can overwrite e.g. by
        // "hdfs://domain.namenode:8020"
        List<String> hdfsAddresses =
            hdfsServerUrls.equals("")
            ? Arrays.asList(tableMeta.getHdfsAddress())
            : Arrays.asList(hdfsServerUrls.split(","));
        ...
        if (hasStaticPartition) {
            // Static partition
            HCatMetadataService.createTablePartitionIfNotExists(
                    hcatDB,
                    hcatTable,
                    staticPartition,
                    hcatAddress,
            ...
            if (hivePartitions.size() != exaColumnNums.length) {
                String exMsg =
                    ...
                    + hivePartitions.size()
                    + " partition columns, but "
                    + exaColumnNums.length
                    + " source partition columns were specified.";
            ...
            for (int i = 0; i < exaColumnNums.length; i++) {
                ...
                dynamicPart
                    .append("/")
                    .append(hivePartitions.get(i).getName())
                    .append("=")
                    .append(iter.getString(colNum));
            }
