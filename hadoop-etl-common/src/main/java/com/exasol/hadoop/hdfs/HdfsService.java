...
public class HdfsService {
  ...
  public static FileSystem getFileSystem(List<String> hdfsURLs, Configuration conf)
    ...
    for (String hdfsURL : hdfsURLs) {
      try {
        ...
        // Dirty hack: The above creation of the filesystem does not actually connect to the hdfs
        // namenode,
        // so it does not fail if the hostname is good but hdfs is not listening on the specified
        // port.
  ...
  static List<String> getPartitionPaths(
    ...
    List<String> partitionPaths =
        new ArrayList<>(
            1); // Will hold all directories which are part of the filter (only leave-directories at
                // the end)
   ...
   *
   * @param useKerberos
   * @param kerberosHdfsServicePrincipal if kerberos is true, then service principal for Hdfs,
   *     otherwise null or empty string
   */
  public static Configuration getHdfsConfiguration(
      boolean useKerberos, String kerberosHdfsServicePrincipal, boolean enableRPCEncryption) {
