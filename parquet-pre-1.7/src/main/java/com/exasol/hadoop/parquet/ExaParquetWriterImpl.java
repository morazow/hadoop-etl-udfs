...
public class ExaParquetWriterImpl implements ExaParquetWriter {
    ...
    /** Parquet writer implementation using column names and column types. */
    public ExaParquetWriterImpl(
    ...
    /** Parquet writer implementation using schema types. */
    public ExaParquetWriterImpl(
            final List<ExaParquetTypeInfo> schemaTypes,
            final Configuration conf,
            final Path path,
            final String compressionType,
            final ExaIterator exa,
        ...
        this(new MessageType(
                    "hive_schema", ExaParquetWriterImpl.typeInfoToParquetTypes(schemaTypes)),
                schemaTypes.size(),
                conf,
                path,
                compressionType,
    ...
    private static List<Type> typeInfoToParquetTypes(
            final List<ExaParquetTypeInfo> exaParquetTypeInfos) {
            ...
            if (exaType.length != 0) {
                ...
                types.add(
                        new PrimitiveType(
                            ...
                            PrimitiveType.PrimitiveTypeName.valueOf(exaType.primitiveTypeName),
                            exaType.name,
                            exaType.originalType == null ? null
                            : OriginalType.valueOf(exaType.originalType)));
            }
        }
