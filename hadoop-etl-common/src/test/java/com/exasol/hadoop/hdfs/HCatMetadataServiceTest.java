...
public class HCatMetadataServiceTest {
    ...
    public void testGetPartitionPaths() throws Exception {
        ...
        pathsAndContent.put(rootPath + "/y=2001", Arrays.asList(
                    fakeFileStatus(rootPath + "/y=2001/m=01"),
                    fakeFileStatus(rootPath + "/y=2001/m=02")));

        pathsAndContent.put(rootPath + "/y=2002", Arrays.asList(
                    fakeFileStatus(rootPath + "/y=2002/m=01"),
                    fakeFileStatus(rootPath + "/y=2002/m=02"),
                    fakeFileStatus(rootPath + "/y=2002/m=03")));

        pathsAndContent.put(rootPath + "/y=2003", Arrays.asList(
                    fakeFileStatus(rootPath + "/y=2003/m=01"),
                    fakeFileStatus(rootPath + "/y=2003/m=11")));

        String filter = "y=2001/m=01";
        List<String> expectedPaths = Arrays.asList(rootPath + "/y=2001/m=01");
        testGetPartitionPathsScenario(
                partitionColumns, pathsAndContent, rootPath, filter, expectedPaths);

        filter = "y=2003";
        expectedPaths = Arrays.asList(rootPath + "/y=2003/m=01", rootPath + "/y=2003/m=11");
        testGetPartitionPathsScenario(
                partitionColumns, pathsAndContent, rootPath, filter, expectedPaths);
    ...
    private void testGetPartitionPathsScenario(
            List<HCatTableColumn> partitionColumns,
            Map<String, List<FileStatus>> pathsAndContent,
            String rootPath,
            String filter,
            List<String> expectedPaths) throws Exception {
        FakeFileSystem fs = new FakeFileSystem(pathsAndContent);
        List<String> paths = HdfsService.getPartitionPaths(
                fs, rootPath, partitionColumns,
                MultiPartitionFilter.parseMultiFilter(filter));
    ...
    public void testFilterMatcherMulti() {
        List<PartitionFilter> filter =
            MultiPartitionFilter.parseMultiFilter("y=2001/m=01, y=2002/m=02, y=2003");
    ...
    public void testPartitionPathFilter() {
        ...
        List<HCatTableColumn> partitionColumns =
            Arrays.asList(new HCatTableColumn("y", "int"), new HCatTableColumn("m", "int"));

        // Filter, and check result
        List<PartitionFilter> multiFilter =
            MultiPartitionFilter.parseMultiFilter("y=2013/m=01, y=2014/m=02, y=2015");
        List<String> filteredPaths1 = fakeFilterPaths(
                pathsPartition1, new PartitionPathFilter(partitionColumns, multiFilter, 1));
        List<String> expectedFilterd1 =
            Arrays.asList(
                    "/user/hive/warehouse/albums_rc_multi_part/y=2013",
                    "/user/hive/warehouse/albums_rc_multi_part/y=2014",
                    "/user/hive/warehouse/albums_rc_multi_part/y=2015");
        assertEquals(expectedFilterd1, filteredPaths1);
        List<String> filteredPaths2 = fakeFilterPaths(
                pathsPartition2, new PartitionPathFilter(partitionColumns, multiFilter, 2));
        List<String> expectedFilterd2 =
            Arrays.asList(
                    "/user/hive/warehouse/albums_rc_multi_part/y=2013/m=01",
                    "/user/hive/warehouse/albums_rc_multi_part/y=2014/m=02",
                    "/user/hive/warehouse/albums_rc_multi_part/y=2015/m=11",
        ...
        List<String> filteredPaths3 = fakeFilterPaths(
                pathsPartition2, new PartitionPathFilter(partitionColumns, multiFilter, 2));
        List<String> expectedFilterd3 =
            Arrays.asList("/user/hive/warehouse/albums_rc_multi_part/y=2013/m=01");
    ...
    public void testPartitionPathFilterInvalidDirs() {
        ...
        List<String> filteredPaths = fakeFilterPaths(
                pathsPartition1, new PartitionPathFilter(partitionColumns, multiFilter, 1));
        List<String> expectedFilterd =
            Arrays.asList("/user/hive/warehouse/albums_rc_multi_part/y=2000");
        assertEquals(expectedFilterd, filteredPaths);
        filteredPaths = fakeFilterPaths(
                pathsPartition2, new PartitionPathFilter(partitionColumns, multiFilter, 2));
        expectedFilterd =
            Arrays.asList(
                    "/user/hive/warehouse/albums_rc_multi_part/y=2000/m=01",
        ...
        filteredPaths = fakeFilterPaths(
                pathsPartition1, new PartitionPathFilter(partitionColumns, multiFilter, 1));
        expectedFilterd = Arrays.asList("/user/hive/warehouse/albums_rc_multi_part/y=2000");
        assertEquals(expectedFilterd, filteredPaths);
        filteredPaths = fakeFilterPaths(
                pathsPartition2, new PartitionPathFilter(partitionColumns, multiFilter, 2));
        expectedFilterd = Arrays.asList("/user/hive/warehouse/albums_rc_multi_part/y=2000/m=02");
        assertEquals(expectedFilterd, filteredPaths);
    }

    private static List<String> fakeFilterPaths(
            List<String> paths, PartitionPathFilter pathFilter) {
