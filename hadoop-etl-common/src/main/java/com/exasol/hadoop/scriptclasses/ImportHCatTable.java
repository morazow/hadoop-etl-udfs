    ...
    if (importSpec.isSubselect()) {
      if (importSpec.getSubselectColumnNames().size() > 0) {
        ...
        for (int i = 0; i < importSpec.getSubselectColumnNames().size(); i++) {
          ...
          emitsBuilder.append(
              "\"" + colName + "\" " + importSpec.getSubselectColumnSqlTypes().get(i));
          first = false;
        }
        emitsBuilder.append(")");
  ...
  static void checkAuthParameters(
    ...
    if (useKerberos) {
      if (!hdfsUser.isEmpty()
        ...
        throw new RuntimeException(
            "If authentication type kerberos is specified, the properties KERBEROS_HCAT_SERVICE_PRINCIPAL and KERBEROS_HDFS_SERVICE_PRINCIPAL have to be specified and HDFS_USER and HCAT_USER must be empty.");
      }
    } else {
      if (hdfsUser.isEmpty()
          || hcatUser.isEmpty()
          ...
          || !kerberosHdfsServicePrincipal.isEmpty()) {
        throw new RuntimeException(
            "If standard basic authentication type is specified (no kerberos), the properties HDFS_USER and HCAT_USER have to be specified and KERBEROS_HCAT_SERVICE_PRINCIPAL and KERBEROS_HDFS_SERVICE_PRINCIPAL must be empty.");
      }
    }
  }
