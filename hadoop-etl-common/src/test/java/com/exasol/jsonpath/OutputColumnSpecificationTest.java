  ...
  public void testParseOutputSpecification() {
    ...
    expected.add(
        new OutputColumnSpec(
            0, "c1", 0, Arrays.asList((JsonPathElement) new JsonPathFieldElement("f1"))));
    expected.add(
        new OutputColumnSpec(
            1,
            "c2",
            1,
            Arrays.asList(
                (JsonPathElement) new JsonPathFieldElement("f1"),
                new JsonPathListIndexElement(0),
                new JsonPathFieldElement("f2"))));
    expected.add(
        new OutputColumnSpec(
            2, "c3", 2, Arrays.asList((JsonPathElement) new JsonPathListIndexElement(1))));
    expected.add(new OutputColumnSpec(3, "c4", 3));
