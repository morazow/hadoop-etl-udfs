...
public class SqlGeneratorForWhereClause extends SqlGenerator {
    ...
    public String visit(SqlColumn column) {
        ...
        try {
            selectedColumns.add(column.getName() + " "
                    + HiveTableInformation.typeMapping(column.getType().toString()));
    ...
    public String visit(SqlPredicateEqual predicate) throws AdapterException {
        ...
        if (!loadAllPartitions) {
            if (leftNode.getType().equals(SqlNodeType.COLUMN)
                    && rightNode.getType().equals(SqlNodeType.LITERAL_STRING)) {
                SqlColumn sqlColumn = (SqlColumn) leftNode;
                boolean isPartitioned =
                    ColumnAdapterNotes.deserialize(sqlColumn.getMetadata().getAdapterNotes(),
                            sqlColumn.getMetadata().getName()).isPartitionedColumn();
                if (isPartitioned) {
                    selectedPartitions.add(sqlColumn.getName() + "="
                            + ((SqlLiteralString) rightNode).getValue());
                } else {
                    loadAllPartitions = true;
                }
            } else if (rightNode.getType().equals(SqlNodeType.COLUMN)
                    && leftNode.getType().equals(SqlNodeType.LITERAL_STRING)) {
                ...
                if (isPartitioned) {
                    selectedPartitions.add(sqlColumn.getName() + "="
                            + ((SqlLiteralString) leftNode).getValue());
                } else {
                    loadAllPartitions = true;
                }
            }
    ...
    public String visit(SqlPredicateInConstList predicate) throws AdapterException {
        ...
        return expression.accept(this) + " IN (" + Joiner.on(", ").join(argumentsSql) + ")";
    }
