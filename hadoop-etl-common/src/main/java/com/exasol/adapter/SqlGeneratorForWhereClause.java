...
public class SqlGeneratorForWhereClause extends SqlGenerator {
  ...
  public String visit(SqlColumn column) {
    ...
    try {
      selectedColumns.add(
          column.getName() + " " + HiveTableInformation.typeMapping(column.getType().toString()));
    ...
    if (!loadAllPartitions) {
      if (leftNode.getType().equals(SqlNodeType.COLUMN)
        ...
        boolean isPartitioned =
            ColumnAdapterNotes.deserialize(
                    sqlColumn.getMetadata().getAdapterNotes(), sqlColumn.getMetadata().getName())
                .isPartitionedColumn();
        if (isPartitioned) {
          selectedPartitions.add(
              sqlColumn.getName() + "=" + ((SqlLiteralString) rightNode).getValue());
        } else {
        ...
        if (isPartitioned) {
          selectedPartitions.add(
              sqlColumn.getName() + "=" + ((SqlLiteralString) leftNode).getValue());
        } else {
          loadAllPartitions = true;
        }
      }
