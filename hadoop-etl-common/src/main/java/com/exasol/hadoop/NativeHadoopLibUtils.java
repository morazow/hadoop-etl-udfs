...
public class NativeHadoopLibUtils {
    ...
    public static void initHadoopNativeLib()
            throws NoSuchFieldException, SecurityException,
                   IllegalArgumentException, IllegalAccessException {
        // This path must exist at program startup and must be in LD_LIBRARY_PATH. Otherwise the
        // dynamic linker seems to ignore it
    ...
    private static void writeNativeHadoopLibs(String targetLibDir) {
        try {
            ...
            // libhadoop.so does a dlopen("libsnappy.so.1"), so we have to use the same filename
            // here
            writeNativeHadoopLib("libsnappy.so", "libsnappy.so.1", targetLibDir);
        } catch (Exception e) {
            System.out.println(
                "Error writing the native Hadoop libraries from resources to tmp: "
                + e.toString());
            e.printStackTrace();
        }
    }

    private static void writeNativeHadoopLib(
            String resourceName, String targetFileName, String targetPath) throws Exception {
        ...
        if (dirFile.exists()) {
            throw new RuntimeException(
                ...
                + targetPath
                + targetFileName
                + " for native Hadoop libraries (file already exists)");
        }
     ...
     * which is required to load java native libraries.
     *
     * <p>From a Sun engineer at http://forums.sun.com/thread.jspa?threadID=707176 See
     ...
     */
    private static void addDirToJavaLibraryPath(String path)
    ...
    /**
     * This is another variant of addDirToJavaLibraryPath().
     *
     * <p>Currently unused, because the other should work.
     */
