...
/**
 * Utility Methods to work with a multi partition filter, for example,
 * user specified multiple filters (logically connected by OR).
 */
public class MultiPartitionFilter {
    ...
    public static boolean matchesAnyFilter(
            Map<String, String> partitionValues, List<PartitionFilter> filters) {
        if (filters.isEmpty()) {
            return true;
        }
    ...
    private static boolean matchesSingleFilter(
            Map<String, String> partitionValues, PartitionFilter filter) {
        ...
        for (String partition : partitionValues.keySet()) {
            if (filter.containsPartition(partition)
                    && !filter.getPartitionValue(partition)
                              .equals(partitionValues.get(partition))) {
                return false;
            }
        }
        return true;
