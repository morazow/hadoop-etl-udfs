...
public class Tuple {
    ...
    public Tuple getTuple(int index) {
        ...
        if (data[index] instanceof Tuple) {
            ...
            throw new RuntimeException(
                    "Object at index " + index + " is of type "
                    + data[index].getClass().getName() + ", not of type Tuple");
        }
    }

    /** Check if is null at the index. */
    ...
    public void writePrimitiveValue(
            RecordConsumer recordConsumer, int index, PrimitiveType primitiveType) {
        if (index >= data.length) {
            throw new RuntimeException("Tuple index " + index + " is out of bounds.");
        }
            ...
            switch (primitiveTypeName) {
                ...
                case FIXED_LEN_BYTE_ARRAY:
                    if (originalType == null || // If not set, assume it's OK
                        ...
                        throw new RuntimeException(
                                "Unsupported FIXED_LEN_BYTE_ARRAY, original type: "
                                + originalType);
                    }
                    break;
                case BINARY:
                ...
                case INT96:
                    ...
                    String timestampStr =
                        ...
                        + String.format("%09d", timestamp.getNanos()) + " +0000";
                    DateTimeFormatter dateTimeFormat =
                        DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSSSSSSSS Z");
                    ZonedDateTime zonedDateTime =
                        ZonedDateTime.parse(timestampStr, dateTimeFormat);
                    long dayNanoSeconds =
