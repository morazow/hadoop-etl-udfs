...
/**
 * Retrieves files from (web)HDFS and decodes them using the appropriate Hadoop InputFormat and Hive
 * SerDe.
 *
 * <p>TODO org.apache.hadoop.hive.serde2.SerDe is deprecated and should be replaced with
 * org.apache.hadoop.hive.serde2.AbstractSerDe Problem: Some SerDes like OrcInputFormat support only
 * deprecated SerDe class (not AbstractSerDe)
 */
public class HdfsSerDeImportService {
  public static void importFiles(
    ...
    try {
      ...
      final List<HCatTableColumn> partitionColumns =
          WebHCatJsonParser.parseColumnArray(partitionColumnsJson);
      final List<HCatSerDeParameter> serDeParameters =
          WebHCatJsonParser.parseSerDeParameters(serdeParametersJson);

      // Parse Output Column Spec
      final List<OutputColumnSpec> outputColumns =
          outputColumnsSpec.isEmpty()
              ? OutputColumnSpecUtil.generateDefaultOutputSpecification(columns, partitionColumns)
              : OutputColumnSpecUtil.parseOutputSpecification(
                  outputColumnsSpec, columns, partitionColumns);
      ...
      UserGroupInformation ugi =
          useKerberos
              ...
              : UserGroupInformation.createRemoteUser(hdfsUserOrServicePrincipal);
      ugi.doAs(
  ...
  /**
   * @return In case of an HA failover, this method might connect to a different namenode, which is
   *     then returned
   */
  public static FileSystem importFile(
    ...
    // If there is a HA namenode failover while UDFs are running, it will probably crash here when
    // retrieving the files from namenode.
    // In case of a crash, we call {@HdfsService#getFileSystem} which will get the available
    // namenode (or it fails because all are down)
    try {
    ...
    CompressionCodecFactory codecs =
        new org.apache.hadoop.io.compress.CompressionCodecFactory(conf);
    ...
    if (!codecList.isEmpty()) CompressionCodecFactory.setCodecClasses(conf, codecList);
    ...
    String decodedFilePath =
        URLDecoder.decode(
            file,
            "UTF-8"); // Convert from URL encoding. TODO Probably we should only do this for webHDFS
    ...
    Map<Integer, List<OutputColumnSpec>> outColsForColumns =
        OutputColumnSpecUtil.getOutputSpecsForColumns(outputColumns);
    while (rr.next(key, value)) {
      // In complex type hierarchies with null values, some columns might not be set, so we have to
      // set manually.
      // TODO Find a better which is optimized for the case where we have to nullify only the fields
      // which were not already overwritten.
      ...
      for (int i = 0; i < columns.size(); i++) {
        ...
        try {
          ...
          visitTree(
              ...
              curPath,
              outColsForColumns.get(i),
              objs,
              jsonFactory);
        } catch (ArrayIndexOutOfBoundsException e) {
          // This is a fix for Hive Versions earlier 2.0 which don't have this path:
          // https://issues.apache.org/jira/browse/HIVE-12475
          // This can be reproduced by creating a Parquet table, inserting data, and afterwards
          // adding columns.
          // Then, when reading the files created before the columns were added, you get the
          // Exception.
          objs[i] = null;
        } catch (Exception e) {
  ...
  private static void visitTree(
    ...
    if (data == null) {
      // If we have vectorized processing, we should go further visiting the tree via
      // ObjectInspector (if any of the elements in the array has a non-null value)
      return;
    }
    ...
    for (OutputColumnSpec spec : outputColumns) {
      if (spec.getPath().equals(curPath)) {
        switch (objInspector.getCategory()) {
          case PRIMITIVE:
            // We reached a leaf. Path should match completely (We could show an error if there are
            // deeper paths for this subtree)
            outRow[spec.getResultPosition()] = getJavaObjectFromPrimitiveData(data, objInspector);
            break;
          case LIST:
            outRow[spec.getResultPosition()] =
                getJsonArrayFromFieldData(data, objInspector, jsonFactory).build().toString();
            break;
          case MAP:
          case STRUCT:
          case UNION:
            outRow[spec.getResultPosition()] =
                getJsonObjectFromFieldData(data, objInspector, jsonFactory).build().toString();
            break;
        }
      }
    }
    ...
    while (objInspector.getCategory() == Category.UNION) {
      // Type may vary for each row, we have to be careful. If we vectorize and we find a union we
      // have to set vector size to 1.
    ...
    // Check if we need to go deeper, i.e. if there is any output column with a path below the
    // current path
    ...
    switch (objInspector.getCategory()) {
      ...
      default:
        {
          throw new RuntimeException(
              "Unexpected category " + objInspector.getCategory() + ", should never happen");
        }
    }
  }
  ...
  private static void initProperties(
    ...
    if (colNames.length() > 0) colNames = colNames.substring(0, colNames.length() - 1);
    if (colTypes.length() > 0) colTypes = colTypes.substring(0, colTypes.length() - 1);
  ...
  private static Object getJavaObjectFromFieldData(Object data, ObjectInspector objInsp) {
    ...
    if (objInsp.getCategory() == Category.PRIMITIVE) {
      ...
      return getJsonArrayFromFieldData(data, objInsp, Json.createBuilderFactory(null))
          .build()
          .toString();
    } else {
      return getJsonObjectFromFieldData(data, objInsp, Json.createBuilderFactory(null))
          .build()
          .toString();
    }
  }

  private static JsonArrayBuilder getJsonArrayFromFieldData(
      Object data, ObjectInspector objInsp, JsonBuilderFactory jsonFactory) {
    ...
    for (Object obj : list) {
      if (obj == null) jab.addNull();
      else if (elemInsp.getCategory() == Category.PRIMITIVE) {
        Object o = getJavaObjectFromPrimitiveData(obj, elemInsp);
        if (o instanceof Integer || o instanceof Short || o instanceof Byte) jab.add((Integer) o);
        else if (o instanceof Long) jab.add((Long) o);
        else if (o instanceof Float || o instanceof Double) jab.add((Double) o);
        else if (o instanceof BigDecimal) jab.add((BigDecimal) o);
        else if (o instanceof Boolean) jab.add((Boolean) o);
  ...
  private static JsonObjectBuilder getJsonObjectFromFieldData(
      Object data, ObjectInspector objInsp, JsonBuilderFactory jsonFactory) {
      ...
      case MAP:
        {
          ...
          for (Map.Entry<?, ?> entry : map.entrySet()) {
            if (keyInsp.getCategory() != Category.PRIMITIVE)
              throw new RuntimeException(
                  "Hive map key is not a primitve type: " + keyInsp.getCategory());
            String key = getJavaObjectFromFieldData(entry.getKey(), keyInsp).toString();
            addJsonObjectPair(
                job, key, entry.getValue(), oi.getMapValueObjectInspector(), jsonFactory);
          }
          break;
        }
      case STRUCT:
        {
          ...
          for (int i = 0; i < size; i++) {
            ...
            String name = field.getFieldName();
            addJsonObjectPair(
                job, name, structValues.get(i), field.getFieldObjectInspector(), jsonFactory);
          }
          break;
        }
      case UNION:
        {
          ...
          addJsonObjectPair(
              ...
              Byte.toString(tag),
              oi.getField(data),
              oi.getObjectInspectors().get(tag),
              jsonFactory);
          break;
        }
      default:
  ...
  private static void addJsonObjectPair(
      ...
      JsonBuilderFactory jsonFactory) {
    if (obj == null) job.addNull(key);
    else if (objInsp.getCategory() == Category.PRIMITIVE) {
      Object o = getJavaObjectFromFieldData(obj, objInsp);
      if (o instanceof Integer) job.add(key, (Integer) o);
      else if (o instanceof Byte) job.add(key, (Byte) o);
      else if (o instanceof Short) job.add(key, (Short) o);
      else if (o instanceof Long) job.add(key, (Long) o);
      else if (o instanceof Float) job.add(key, (Float) o);
      else if (o instanceof Double) job.add(key, (Double) o);
      else if (o instanceof BigDecimal) job.add(key, (BigDecimal) o);
      else if (o instanceof Boolean) job.add(key, (Boolean) o);
      else job.add(key, o.toString());
    } else if (objInsp.getCategory() == Category.LIST) {
  ...
  private static void setPartitionValues(
      String file,
      int numRegularColumns,
    ...
    for (OutputColumnSpec outputColumn : outputColumns) {
      if (outputColumn.getColumnPosition() >= numRegularColumns) {
        HCatTableColumn partitionColumn =
            partitionColumns.get(outputColumn.getColumnPosition() - numRegularColumns);
        String partName = partitionColumn.getName();
        String partPrefix = "/" + partName + "=";
        int partIdx = file.indexOf(partPrefix);
        if (partIdx < 0) {
          throw new RuntimeException(
              "Hive partition '" + partName + "' could not be found in path: " + file);
        }
        partIdx += partPrefix.length();
        int slashIdx = file.indexOf('/', partIdx);
        if (slashIdx < 0) {
          throw new RuntimeException(
              "End of value for Hive partition '"
                  + partName
                  + "' could not be found in path: "
                  + file);
        }
        String value = file.substring(partIdx, slashIdx);
        objs[outputColumn.getResultPosition()] =
            getJavaObjectFromHiveString(value, partitionColumn.getDataType());
      }
    }
  }
