...
public class WebHdfsAndHCatService {
  ...
  public static String getExtendedTableInfo(
      String webHCatServer, String dbName, String tableName, String hcatUser) throws Exception {
    String requestPath = "ddl/database/" + dbName + "/table/" + tableName;
    String requestQuery = "format=extended";
    System.out.println(
        "Retrieving table infos now. Request: " + "" + requestPath + " query: " + requestQuery);
    return get(
        WebHdfsAndHCatService.Service.HCAT, webHCatServer, requestPath, requestQuery, hcatUser);
  }
  ...
  public static String get(
    ...
    try {
      ...
      throw new RuntimeException(
          "Problem accessing web" + service.toString() + " (" + urlStr + "): " + e.getMessage(), e);
    } finally {
      if (httpConn != null) {
        httpConn.disconnect();
      }
