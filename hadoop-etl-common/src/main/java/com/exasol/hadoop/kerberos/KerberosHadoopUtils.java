...
public class KerberosHadoopUtils {
    ...
    public static UserGroupInformation getKerberosUgi(
        ...
        UserGroupInformation ugi = UserGroupInformation.loginUserFromKeytabAndReturnUGI(
                kerberosCredentials.getPrincipal(), keytabPath);
    ...
    /**
     * Configure JAAS for Export if JDBC statements must be executed
     * using Kerberos authentication.
     */
    public static void configKerberosJaas(
            String path, String user, String password) throws Exception {
        ...
        String keytabPath = UdfUtils.writeTempFile(
                DatatypeConverter.parseBase64Binary(confKeytab[2]), path, "kt_", ".keytab");
        ...
        String jaasPath = UdfUtils.writeTempFile(
                jaasData.toString().getBytes(Charset.forName("UTF-8")), path, "jaas_", ".conf");
        String confPath = UdfUtils.writeTempFile(
                DatatypeConverter.parseBase64Binary(confKeytab[1]), path, "krb5_", ".conf");
