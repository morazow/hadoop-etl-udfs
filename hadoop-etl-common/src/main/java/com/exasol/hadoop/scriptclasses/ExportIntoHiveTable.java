...
public class ExportIntoHiveTable {
  ...
  public static void run(ExaMetadata meta, ExaIterator iter) throws Exception {
    ...
    String dynamicPartitionExaCols =
        UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_DYNAMIC_PARTITION_EXA_COLS, "");
    ...
    boolean enableRPCEncryption =
        UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_ENABLE_RPC_ENCRYPTION, "false")
    ...
    HCatTableMetadata tableMeta =
        HCatMetadataService.getMetadataForTable(
            hcatDB,
            hcatTable,
            hcatAddress,
    ...
    if (!staticPartition.isEmpty() && !dynamicPartitionExaCols.isEmpty()) {
      ...
      if (hivePartitions.isEmpty()) {
        throw new RuntimeException(
            "A static partition was specified although the target table does not have any partitions.");
      }
      ...
      throw new RuntimeException(
          "A dynamic partition was specified although the target table does not have any partitions.");
    }
    ...
    // "hdfs://namenode:8020")
    // Two use cases: 1) Use webHDFS instead of HDFS and 2) If Hadoop returns a namenode hostname
    // unreachable from EXASOL (e.g. not fully-qualified) we can overwrite e.g. by
