...
public class ImportHiveTableFiles {
    ...
    public static void run(ExaMetadata meta, ExaIterator iter) throws Exception {
        ...
        final String inputFormatClassName = iter.getString(PARAM_IDX_INPUT_FORMAT);
        final String serDeClassName = iter.getString(PARAM_IDX_SERDE);
        final String colInfo = iter.getString(PARAM_IDX_COLUMNS);
        final String partInfo = iter.getString(PARAM_IDX_PARTITIONS);
        final String serdeProps = iter.getString(PARAM_IDX_SERDE_PROPS);
        final String hdfsServerUrls = iter.getString(PARAM_IDX_HDFS_ADDRESS);
        final String hdfsUser = iter.getString(PARAM_IDX_HDFS_USER);
        final List<String> hdfsAddresses = Arrays.asList(hdfsServerUrls.split(","));
        ...
        Boolean enableRpcEncryption = UdfUtils.getOptionalStringParameter(
        ...
        if (!debugAddress.isEmpty()) {
            try {
                ...
                throw new RuntimeException("You have to specify a valid hostname and port for "
                        + "the udf debug service, e.g. 'hostname:3000'");
            }
        }
        ...
        HdfsSerDeImportService.importFiles(
                files,
