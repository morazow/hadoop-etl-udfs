...
public class HCatTableFiles {
    ...
    public static void run(ExaMetadata meta, ExaIterator iter) throws Exception {
        ...
        final String hcatDB = iter.getString(PARAM_IDX_HCAT_DB);
        final String hcatTable = iter.getString(PARAM_IDX_HCAT_TABLE);
        final String hCatAddress = iter.getString(PARAM_IDX_HCAT_ADDRESS);
        ...
        final String partitionFilterSpec =
            UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_PARTITIONS, "");
        String outputColumnsSpec =
            UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_OUTPUT_COLUMNS, "");
        String hdfsAddressesFromUser =
        ...
        String connName =
            UdfUtils.getOptionalStringParameter(meta, iter, PARAM_IDX_KERBEROS_CONNECTION, "");
        ...
        if (!tableType.equalsIgnoreCase("MANAGED_TABLE")
                && !tableType.equalsIgnoreCase("EXTERNAL_TABLE")) {
            throw new RuntimeException(
                    "Table type "
                    + tableType
                    + " is not supported. Only managed and external tables are supported.");
        }
        ...
        for (int i = 0; i < numFilePaths; i++) {
            iter.emit(StringUtils.join(hdfsAddresses, ","),
                    filePaths.get(i),
                    hdfsUserOrServicePrincipal,
                    hcatUserOrServicePrincipal,
                    ...
                    WebHCatJsonSerializer.serializeSerDeParameters(
                        tableMeta.getSerDeParameters()),
                    i % parallelism,
                    authType,
                    connName,
                    outputColumnsSpec,
